...
void TreeSkeleton :: paint(long long now)
{
    ...
    while(index < total_branches && index > 0)
    {
        ...
        if(!rendered[index])
        {
			...
			else if(Branch::enablebeta)
			{
				double bent_beta_modifier = (1.0 + 200.0 * (fabs((double) branches[index].current_angle[0])) / (double) branches[index].max_bent_angle[0]);
				//double valindex = fmod(bent_beta_modifier * val, 1.0) * ((double) (branches[index].betasize-1));
				//std::cout << "BENT : " << bent_beta_modifier << "\t\t\t\tVALINDEX : " << valindex << std::endl;
				...
				glRotated(branches[index].current_angle[0] * (180.0/M_PI) + bent_beta_modifier * BRANCH_BETA_ROTATION_LIMIT_X * linearinterpol, 1, 0, 0);
					glRotated(branches[index].current_angle[2] * (180.0/M_PI) + bent_beta_modifier * BRANCH_BETA_ROTATION_LIMIT_Z * linearinterpol, 0, 0, 1);
			}
			else
			{
